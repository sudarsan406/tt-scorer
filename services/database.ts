...
class DatabaseService {
  ...
  /**
   * Edit the score of a specific set in a completed match
   * This will recalculate the match winner and update Elo ratings
   */
  async editGameSetScore(setId: string, player1Score: number, player2Score: number): Promise<void> {
    if (!this.db) throw new Error('Database not initialized');

    const now = new Date().toISOString();

    // Get the set details to find the match
    const set = await this.db.getFirstAsync(
      `SELECT * FROM game_sets WHERE id = ?`,
      [setId]
    ) as any;
    ...
    // Determine new set winner
    const winnerId = player1Score > player2Score ? set.match_id : set.match_id;

    // Get match to determine actual winner IDs
    const match = await this.db.getFirstAsync(
    ...
    const newWinnerId = player1Score > player2Score ? match.player1_id : match.player2_id;

    // Update the set
    await this.db.runAsync(
      `UPDATE game_sets SET player1_score = ?, player2_score = ?, winner_id = ?, updated_at = ? WHERE id = ?`,
      [player1Score, player2Score, newWinnerId, now, setId]
    );

    // Recalculate match winner and update match score
    await this.recalculateMatchWinner(set.match_id);
  }

  /**
   * Recalculate match winner based on current set scores
   * Updates match score, winner, and Elo ratings
   */
  async recalculateMatchWinner(matchId: string): Promise<void> {
    if (!this.db) throw new Error('Database not initialized');

    // Get all sets for this match
    const sets = await this.db.getAllAsync(
      `SELECT * FROM game_sets WHERE match_id = ? ORDER BY set_number ASC`,
      [matchId]
    ) as any[];
    ...
    // Count set wins for each player
    let player1Sets = 0;
    let player2Sets = 0;
    ...
    sets.forEach((set: any) => {
      if (set.winner_id === match.player1_id) {
        player1Sets++;
      } else if (set.winner_id === match.player2_id) {
        player2Sets++;
      }
    });

    // Determine match winner
    const newWinnerId = player1Sets > player2Sets ? match.player1_id : match.player2_id;
    ...
    await this.db.runAsync(
      `UPDATE matches SET player1_sets = ?, player2_sets = ?, winner_id = ?, updated_at = ? WHERE id = ?`,
      [player1Sets, player2Sets, newWinnerId, now, matchId]
    );

    // If winner changed, recalculate Elo ratings
    if (oldWinnerId && newWinnerId !== oldWinnerId) {
      console.log(`Match winner changed from ${oldWinnerId} to ${newWinnerId}, recalculating Elo ratings`);
      await this.recalculatePlayerRatingsForEditedMatch(matchId, oldWinnerId, newWinnerId);
    } else if (!oldWinnerId) {
      // First time setting winner
      await this.updatePlayerRatings(matchId);
    } else {
      // Winner stayed same but scores changed, recalculate with new scores
      await this.updatePlayerRatings(matchId);
    }

    // Update tournament match if this is a tournament match
    await this.updateTournamentMatchAfterEdit(matchId, player1Sets, player2Sets, newWinnerId, oldWinnerId);
  }

  /**
   * Recalculate Elo ratings when a match result is edited
   * Reverses the old rating change and applies the new one
   */
  private async recalculatePlayerRatingsForEditedMatch(
    matchId: string,
    oldWinnerId: string,
    newWinnerId: string
  ): Promise<void> {
    ...
    // Calculate new ratings (this automatically uses the current match result)
    const player1Won = newWinnerId === match.player1_id;
    const eloResult = EloRatingService.calculateNewRatingsWithScore(
      match.player1_rating,
      match.player2_rating,
      player1Won,
    ...
    await this.db.runAsync(
      'UPDATE players SET rating = ?, updated_at = ? WHERE id = ?',
      [eloResult.player1NewRating, now, match.player1_id]
    );

    await this.db.runAsync(
      'UPDATE players SET rating = ?, updated_at = ? WHERE id = ?',
      [eloResult.player2NewRating, now, match.player2_id]
    );

    console.log(`Elo ratings recalculated after score edit: Player 1: ${match.player1_rating} → ${eloResult.player1NewRating}, Player 2: ${match.player2_rating} → ${eloResult.player2NewRating}`);
  }

  /**
   * Update tournament bracket when a match result is edited
   */
  private async updateTournamentMatchAfterEdit(
    matchId: string,
    player1Sets: number,
    player2Sets: number,
    newWinnerId: string,
    ...
    // Find tournament match linked to this match
    ...
    if (!tournamentMatch) {
      // Not a tournament match, nothing to update
      return;
    }

    const now = new Date().toISOString();

    // Update tournament match scores
    await this.db.runAsync(
      `UPDATE tournament_matches SET player1_sets = ?, player2_sets = ?, winner_id = ?, updated_at = ? WHERE id = ?`,
      [player1Sets, player2Sets, newWinnerId, now, tournamentMatch.id]
    );

    // If winner changed, we need to update bracket progression
    if (oldWinnerId && newWinnerId !== oldWinnerId) {
      console.warn('Tournament match winner changed - manual bracket review may be needed');
      // Note: Automatically updating bracket progression could be complex
      // as it may affect subsequent matches that have already been played
      // For safety, we just update the current match and log a warning
    }
  }
